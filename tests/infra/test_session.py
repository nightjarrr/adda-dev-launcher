...
def test_fssessionrepository_create_creates_runtime_dir(tmp_path: Path) -> None:
    repo = FsSessionRepository(runtime_base=tmp_path)
    session = repo.create("demo")
    assert session.runtime_dir.is_dir()


def test_fssessionrepository_create_sets_dir_mode_0o700(tmp_path: Path) -> None:
    repo = FsSessionRepository(runtime_base=tmp_path)
    session = repo.create("demo")
...
def test_fssessionrepository_create_writes_session_toml(tmp_path: Path) -> None:
    repo = FsSessionRepository(runtime_base=tmp_path)
    session = repo.create("demo")
...
def test_fssessionrepository_create_session_id_has_session_prefix(tmp_path: Path) -> None:
    repo = FsSessionRepository(runtime_base=tmp_path)
    session = repo.create("demo")
    assert session.session_id.startswith("session-")


def test_fssessionrepository_create_started_at_is_utc_aware(tmp_path: Path) -> None:
    repo = FsSessionRepository(runtime_base=tmp_path)
    session = repo.create("demo")
...
def test_fssessionrepository_create_sets_project_name(tmp_path: Path) -> None:
    repo = FsSessionRepository(runtime_base=tmp_path)
    session = repo.create("my-project")
    assert session.project_name == "my-project"


def test_fssessionrepository_create_sets_issue_id(tmp_path: Path) -> None:
    repo = FsSessionRepository(runtime_base=tmp_path)
    session = repo.create("demo", issue_id=99)
    assert session.issue_id == 99


def test_fssessionrepository_create_sets_runtime_dir_under_base(tmp_path: Path) -> None:
    repo = FsSessionRepository(runtime_base=tmp_path)
    session = repo.create("demo")
...
def test_fssessionrepository_create_none_issue_id_omitted_from_toml(tmp_path: Path) -> None:
    repo = FsSessionRepository(runtime_base=tmp_path)
...
def test_fssessionrepository_terminate_removes_runtime_dir(tmp_path: Path) -> None:
    repo = FsSessionRepository(runtime_base=tmp_path)
    session = repo.create("demo")
...
def test_fssessionrepository_terminate_is_idempotent(tmp_path: Path) -> None:
    repo = FsSessionRepository(runtime_base=tmp_path)
    session = repo.create("demo")
    repo.terminate(session)
