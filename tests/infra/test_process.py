...
def test_defaultrunner_env_none_inherits_environment() -> None:
    runner = DefaultRunner()
    handle = runner.start(["env"], env=None)
    assert handle.wait() == 0
...
def test_capturedoutputrunner_stdout_returns_captured_output() -> None:
    runner = CapturedOutputRunner()
    handle = runner.start(["echo", "hello"])
    handle.wait()
...
def test_capturedoutputrunner_stdout_none_before_wait() -> None:
    runner = CapturedOutputRunner()
    handle = runner.start(["true"])
...
def test_capturedoutputrunner_stderr_none_before_wait() -> None:
    runner = CapturedOutputRunner()
    handle = runner.start(["true"])
